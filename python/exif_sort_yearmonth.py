...
def get_exif(fn):
	...
	try:
		...
		#raise IOError
			logging.warn("could not open %s", fn)
			return
	info = i._getexif()
...
def sortPhotos(path):
	logging.info("Beginning to sort photos in %s", path)
	PHOTOS = []
	...
	logging.info("Found %d photos to sort", len(PHOTOS))
	for PHOTO in PHOTOS:
		try:
